...
tags:
  ...
  - algebraic-geometry
created: 2026-01-19
source: "Eleven-source boundary audit completed; core topic requires an authoritative motives source"
...
> 1. **Lefschetz standard conjecture**: the inverse to the hard-Lefschetz isomorphism is induced by an algebraic correspondence
> 2. **Hodge standard conjecture**: a specified bilinear form is positive definite on primitive algebraic classes
> 3. **Künneth standard conjecture**: The Künneth components of the diagonal are algebraic

The implications depend on the chosen adequate equivalence relation. For example, the standard conjectures relate homological and numerical equivalence and provide algebraic projectors; semisimplicity must be stated for a specified category (the category of numerical motives with rational coefficients is semisimple by a separate theorem of Jannsen).
...
> [!info] Definition
> In a conjectural abelian category of **mixed motives**, an object $M$ should carry a **weight filtration**:
> $$
...
> Deligne–Goncharov construct and study suitable categories of mixed Tate motives over rings of $S$-integers. Precise claims identifying their period algebras with spaces generated by multiple zeta values require careful choices and a dedicated source; the blanket equality formerly stated here is not asserted.

## The Motivic Galois Group

> [!info] Definition
> After choosing a neutral Tannakian category of motives and a fiber functor $\omega$, its **motivic Galois group** is the Tannakian automorphism group $\operatorname{Aut}^{\otimes}(\omega)$. Without those choices there is no single unconditional group $\mathcal G_{\mathrm{mot}}(k)$.

For a standard neutral Tannakian category of mixed Tate motives over $\mathbb Q$, one expects or obtains—depending on the precise category—a structure of the form
$$
...
> $$
> where, at a good prime and in a fixed arithmetic/geometric Frobenius convention,
> $$
> L_p(M,s)=\det\!\left(1-\operatorname{Frob}_p p^{-s}\mid H_\ell(M)^{I_p}\right)^{-1}.
> $$
...
| Functional equation | Relates the completed L-function of $M$ to that of a dual/Tate twist; the reflected argument depends on weight and normalization (often $w+1-s$ for a pure weight-$w$ self-dual normalization) |
...
Eleven-source boundary audit: Milne defines algebraic cycles, Chow groups, cycle maps, and Tate twists, and briefly mentions mixed Tate motives only in a reference-level aside [S8, §§3 and 23, especially printed/PDF pp. 30 and 138–143]. Kac and Hall add no theory of motives. Getz–Hahn [S11] uses motivic language only as Langlands-program context and does not construct a category of pure or mixed motives. These ingredients are prerequisites, not a definition of a motivic category. The standard conjectures, mixed-motive caveat, Tannakian dependence, Euler-factor inversion, and functional-equation normalization have been corrected, but none of the eleven sources verifies the resulting theory; the note remains `unverified`.
