...
created: 2026-01-19
source: "Claus Sorensen, From Classical L-Functions to Modern Reciprocity Laws, Ch. 5, introduction and §5.7.5, printed pp. 184–185, 233, PDF pp. 193–194, 242"
...
> [!example] Automorphic representations of GL₂
> For $G = \text{GL}_2$ over $\mathbb{Q}$ (with the holomorphic row understood in weight $k\ge2$):
>
...
> [!example] Discrete isobaric representations
> In the discrete automorphic spectrum of $\text{GL}_n$, isobaric sums are built from cuspidal data:
> $$
...
> [!abstract] Theorem (Godement–Jacquet; schematic form)
> For cuspidal $\pi$ on $\text{GL}_n$ with $n\ge 2$:
> 1. The completed standard $L$-function extends to an entire function
> 2. Satisfies a functional equation relating $s$ and $1-s$
> 3. Bounded in vertical strips
>
> For $n=1$, the corresponding statement is Hecke theory: the trivial character gives a pole, while nontrivial characters have the appropriate holomorphy statement.
...
Sorensen places classical cusp forms inside an adelic $L^2$ space carrying the right-regular $\mathrm{GL}_2(\mathbb A)$ action, describes automorphic representations there as the relevant irreducible subquotients, identifies the subrepresentation generated by an adelized modular form as a principal example, and states that a primitive form yields a restricted tensor product $\pi=\bigotimes'_{p\leq\infty}\pi_p$. This verifies the core $\mathrm{GL}_2/\mathbb Q$ motivation and factorization picture. The general reductive-group definition, Flath's theorem, detailed local types, cuspidal/residual/continuous spectra, Godement–Jacquet, Langlands classification, multiplicity one, and the global status table are not proved in the checked pages. The note is `partially-verified`.
